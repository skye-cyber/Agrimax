...
path = 'dataset/synthetic_dataset.csv'
...
# Group by 'crop' and calculate key statistics for numerical columns
key_stats = df.groupby('crop').agg({
    'N': ['mean', 'min', 'max', 'std'],
    'P': ['mean', 'min', 'max', 'std'],
    'K': ['mean', 'min', 'max', 'std'],
...
def train_and_save_model(model, model_name, X_train, y_train, X_test, y_test, features, target, save_path):
    ...
    - y_train: Training crops.
    - X_test: Testing features.
    - y_test: Testing crops.
...
def _DecisionTree():
    ...
    DecisionTree = DecisionTreeClassifier(
        criterion="gini", max_leaf_nodes=90, random_state=2, max_depth=11)  #-> 90.30%

    train_and_save_model(DecisionTree, 'Decision Tree', X_train, y_train,
                         X_test, y_test, features, target, 'DecisionTree.pkl')
...
def _LightGBM():
    ...
    LightGBM = lgb.LGBMClassifier(
        verbose=-1, num_leaves=70, n_estimators=11)  #-> 88.76%
    train_and_save_model(LightGBM, 'Light Gradient Boosting Machine', X_train, y_train,
                         X_test, y_test, features, target, 'LightGBM.pkl')
...
def _RFC():
    ...
    RFC = RandomForestClassifier(
        n_estimators=55, max_leaf_nodes=50_000_000, random_state=42)  # 90.62%
...
def _XGBoost():
    ...
    target_encoded = df_encoded['crop']
    ...
    train_and_save_model(Xboost, 'XGBoost Classifier', X_train, y_train_encoded,
                         X_test, y_test_encoded, features_encoded, target_encoded, 'XGBoost.pkl')


def main():
    parser = argparse.ArgumentParser(
        description="Train Crop_recommendation models")
    parser.add_argument('--model', '-m', choices=["xgboost", "RFC", "LR", "SVM", 'lightgbm', 'decision_tree'], type=str,
